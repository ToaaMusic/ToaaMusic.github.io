...
import { useScrollState } from '../composables/scrollStore'

const props = defineProps<{
  headings: { id: string; text: string; level: number }[]
}>()

const { activeHeadingId, setActiveHeading } = useScrollState()

const activeId = computed(() => activeHeadingId.value)
...
const updateHighlight = () => {
  nextTick(() => {
    ...
    if (!props.headings || !currentActiveId) {
      highlightStyle.value = null
      return
    }

    const index = props.headings.findIndex(h => h.id === currentActiveId)
